...
export type Weather = "sunny" | "cloudy" | "rainy" | "snowy" | "stormy" | "foggy";
...
const calcCalenderGridIndex = (event: CalenderEvent) => {
  ...
  const rowIndex = ((now.getTime() - eventDate.getTime()) / (1000 * 60 * 60 * 24 * 7)) | 0;
  const columnIndex = 6 - eventDate.getDay();

  return `${rowIndex}:${columnIndex}` as `${number}:${number}`;
};

export const calcCalenderEventMap = (events: CalenderEvent[]): CalenderEventMap => {
...
export const getMoodBasedColors = (
  ...
  const hue = 200; // 青
  const saturation = 5;
  const lightness = intensity * 80 + 20; // 20%から100%の範囲
...
export const getCalendarDateStyles = (
  ...
  if (isFutureDate) {
    return {
      className: "w-6 h-6 transition-all duration-300 hover:scale-110 hover:shadow-md border-white cursor-not-allowed",
    };
  }
