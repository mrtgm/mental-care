        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 flex items-center gap-1">
            <div className="w-12 h-1.5 bg-gray-600 rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all ${getVasColor(mood.vas)}`} style={{ width: `${mood.vas}%` }} />
            </div>
            <span className={`text-xs font-medium ${getVasTextColor(mood.vas)}`}>{mood.vas}</span>
          </div>
        </div>
