...
import { calculateTotalScore, getAchievementsByType, getTotalScoreColor } from "./utils/achievements";

const AchievementItem = ({ achievement, type }: { achievement: Achievement; type: Achievement["type"] }) => {
...
export const AchievementsDisplay = ({ achievements }: { achievements: string[] }) => {
  ...
  return (
    <div>
      <div className="text-white text-xs mb-1">{FieldLabelMap.achievements}:</div>

      <div className="ml-2 space-y-1">
        {positiveAchievements.map((achievement, index) => (
          <AchievementItem key={`positive-${achievement.id}`} achievement={achievement} type="positive" />
        ))}

        {negativeAchievements.map((achievement, index) => (
          <AchievementItem key={`negative-${achievement.id}`} achievement={achievement} type="negative" />
        ))}
